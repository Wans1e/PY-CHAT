    ...
    def secure_input(callback: Callable[..., None]) -> Callable[..., Any]:
        def wrapper(*args, **kwargs):
            try:
                ...
                print('Failed creating/joining chatroom, press enter to exit.')
                getpass.getpass('')
                sys.exit()
...
class Main:
    ...
    def __init__(self, lines: int, cols: int, theme: str) -> None:
        ...
        if category == 'create':
            ...
            pubk_filepath  = os.path.join('Certificates', 'public_key.pem')
            privk_filepath = os.path.join('Certificates', 'private_key.pem')
